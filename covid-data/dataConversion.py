...
# Also drop columns no longer needed now
df_ger = df_ger.drop(['location_name', 'date'], axis=1)
df_it = df_it.drop(['location_name', 'date'], axis=1)
df_us = df_us.drop(['location_name', 'date'], axis=1)

# Write new file
df_ger.to_csv("covid-ger.csv", index=False)
df_it.to_csv("covid-it.csv", index=False)
df_us.to_csv("covid-us.csv", index=False)