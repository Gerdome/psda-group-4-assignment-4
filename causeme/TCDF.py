...
def train(epoch, traindata, traintarget, modelname, optimizer,log_interval,epochs, verbose=True):
    ...
    if verbose and (epoch % log_interval ==0 or epoch % epochs == 0 or epoch==1):
...
def findcauses(target, cuda, epochs, kernel_size, layers, 
               log_interval, lr, optimizername, seed, dilation_c, significance, data, verbose=True):
    ...
    scores, firstloss = train(1, X_train, Y_train, model, optimizer,log_interval,epochs, verbose)
    firstloss = firstloss.cpu().data.item()
    for ep in range(2, epochs+1):
        scores, realloss = train(ep, X_train, Y_train, model, optimizer,log_interval,epochs, verbose)
